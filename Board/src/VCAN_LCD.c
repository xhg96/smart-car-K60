 ...
 *  Sample usage:       Site_t site = {10,20};      //x = 10 ,y = 20
                        ...
                        LCD_Img_gray(site, size, img);
 */
void LCD_Img_edge(Site_t site, Size_t size, uint8 *img)
{
    uint8 line = size.H;
	uint8 n = 0;
...
void LCD_Img_edge_Z(Site_t site, Size_t size, uint8 *img, Size_t imgsize)
{
...
void LCD_Img_leftline(Site_t site, Size_t size, uint8 *img)
{
    uint8 line = size.H;
	uint8 n = size.W;
	...
	while(line--)
	{
		...
		for(n = 0; n < size.W; n++)
		{
            ...
            if(imgtemp == 2)
            {
            	...
            	LCD_PTLON(site, size_tmp);    //开窗
    			LCD_RAMWR();			      //写内存

				LCD_WR_DATA( RED );               //写图像数据
            }
        }
    }
}

void LCD_Img_rightline(Site_t site, Size_t size, uint8 *img)
{
    uint8 line = size.H;
	uint8 n = size.W;
...
void LCD_Img_gray_Z(Site_t site, Size_t size, uint8 *img, Size_t imgsize)
{
    ...
    for(y = 0; y < size.H; y++)
    {
        ...
        for(x = 0; x < size.W; x++)
        {
            ...
            // LCD_WR_DATA(rgb);
            if(pimg[temp] == 0) LCD_WR_DATA( WHITE );
            else if(pimg[temp] == 1) LCD_WR_DATA(BLACK  );
            else if(pimg[temp] == 2) LCD_WR_DATA( GREEN );
            else if(pimg[temp] == 3) LCD_WR_DATA( RED );
            else if(pimg[temp] == 4)LCD_WR_DATA( BLUE);
            else if(pimg[temp] == 5)LCD_WR_DATA( RED);
 ...
 *  Sample usage:       Site_t site = {10,20};          //x = 10 ,y = 20
                        ...
                        LCD_Img_gray_Z(site, size, img,imgsize);
 */
void LCD_Img_gray_Z_CCD(Site_t site, Size_t size, uint8 *img, Size_t imgsize)
{
...
void LCD_wave_display(Site_t site,Size_t size,uint8 *img,uint8 maxval,uint16 Color)
{
    ...
    uint8 h, C_h;
    uint16 y = site.y + size.H - 1, C_y = site.y + size.H - 1;
    
    site.x += size.W;
    
    Site_t C_site;
    static uint8 CCD_BUFF_Last[TSL1401_SIZE]; //上次CCD图像
    int C_siteW = size.W;
    C_site.x += size.W;
    ...
    while(C_siteW --)    //清上次的CCD图像
    {
        if(CCD_BUFF_Last[C_siteW] >= maxval )
        {
            C_h = size.H - 1;
        }
        else
        {
            C_h = ((CCD_BUFF_Last[C_siteW] )* size.H )/maxval;
        }
        C_site.y = C_y - C_h ;
        C_site.x--;
        LCD_point(C_site, WHITE);
    }
    
  uint16 i = 0;
  for(i = 0; i<TSL1401_SIZE; i++){      //保留上次CCD图像做显示用
    CCD_BUFF_Last[i] = img[i];
  }
  
    while(size.W --)    //显示现在的CCD图像
    {
        if(*img >= maxval )
        {
            h = size.H - 1;
