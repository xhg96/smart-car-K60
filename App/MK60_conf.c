...
void start_check()
{
    ...
    if(key_check(KEY_B) == KEY_DOWN )
    {
        ...
        while(1)
        {
            ...
            if((flag == 1)  && (key_check(KEY_B) == KEY_DOWN ))
            {
                while(key_check(KEY_B) == KEY_DOWN)         //等待按键 弹起
                {
                    led_turn(LED0);
                    DELAY_MS(100);                          //此处的延时，可起到 按键 消抖的功能
                }

                while(key_check(KEY_B) == KEY_UP)         //等待按键 弹起后 再 进行解锁
                {
                    led_turn(LED0);
