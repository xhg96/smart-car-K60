...
#define VAR_MAX1                    (uint8)(LCD_pagenum * 5)
...
typedef struct
{
    ...
    float changval;          //每次按改变的值
    float holdval;           //按住变化的值

    float showen;            //显示放大倍数

    char* name;              //变量名称
    Site_t namesite;         //名称地址
...
//变量 的 赋值 与 读值
extern void     save_var(var_tab_e var_tal, float var_data);       //保存编号变量的值
extern void     get_var(var_tab_e var_tal, float *var_data);       //获取编号变量的值 (根据变量编号找到变量地址，从而获取变量的值)
